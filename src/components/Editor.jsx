...
const COUNTDOWN_FORMAT_OPTIONS = ['SecondsHundreth', 'MinutesSeconds', 'MinutesSecondsHundreth', 'HoursMinutes', 'HoursMinutesSeconds', 'DaysHoursMinutes', 'DaysHoursMinutesSeconds'];
const SCROLL_MOVEMENT_OPTIONS = ['Unrestricted', 'Elastic', 'Clamped'];
...
const PRESETS = {
  ...
  ScrollView: { color: '0.08 0.12 0.18 0.88', opacity: 100, horizontal: false, vertical: true, movementType: 'Clamped', inertia: true, elasticity: 0.1, decelerationRate: 0.135, scrollSensitivity: 24, contentScaleX: 1, contentScaleY: 1.6, anchor: { min: '0.16 0.16', max: '0.46 0.64' }, offset: { minX: 0, minY: 0, maxX: 0, maxY: 0 }, rotation: 0, align: 'UpperLeft' },
  Countdown: { text: '00:30', color: '#f5fbff', opacity: 100, font: 'RobotoCondensed-Bold.ttf', fontSize: 30, command: '', startTime: 0, endTime: 30, interval: 1, timerFormat: 'MinutesSeconds', destroyIfDone: false, anchor: { min: '0.40 0.64', max: '0.60 0.70' }, offset: { minX: 0, minY: 0, maxX: 0, maxY: 0 }, rotation: 0, align: 'MiddleCenter' },
  Slot: { color: '0.09 0.13 0.18 0.74', opacity: 100, filter: 'any', anchor: { min: '0.12 0.16', max: '0.20 0.30' }, offset: { minX: 0, minY: 0, maxX: 0, maxY: 0 }, rotation: 0, align: 'MiddleCenter' }
};
        ...
        <div className="editor-panel">
          ...
          {!selectedElement ? <div className="editor-empty">Add an element or choose one from the layer list.</div> : (
            <div className="editor-field-grid">
              ...
              {(selectedElement.type === 'Text' || selectedElement.type === 'Button' || selectedElement.type === 'InputField' || selectedElement.type === 'Countdown') && <Field label="Text" full><textarea value={selectedElement.text || ''} onChange={(event) => patchSelected((element) => ({ ...element, text: event.target.value }))} /></Field>}
              ...
              {(selectedElement.type === 'Text' || selectedElement.type === 'Button' || selectedElement.type === 'InputField' || selectedElement.type === 'Countdown') && <>
              ...
              {selectedElement.type === 'ScrollView' && <>
                <Field label="Horizontal"><select value={selectedElement.horizontal ? 'yes' : 'no'} onChange={(event) => patchSelected((element) => ({ ...element, horizontal: event.target.value === 'yes' }))}><option value="no">No</option><option value="yes">Yes</option></select></Field>
                <Field label="Vertical"><select value={selectedElement.vertical === false ? 'no' : 'yes'} onChange={(event) => patchSelected((element) => ({ ...element, vertical: event.target.value === 'yes' }))}><option value="yes">Yes</option><option value="no">No</option></select></Field>
                <Field label="Content Width"><input type="number" min="1" step="0.1" value={selectedElement.contentScaleX || 1} onChange={(event) => patchSelected((element) => ({ ...element, contentScaleX: Math.max(1, roundTo(toNumber(event.target.value, 1), 2)) }))} /></Field>
                <Field label="Content Height"><input type="number" min="1" step="0.1" value={selectedElement.contentScaleY || 1} onChange={(event) => patchSelected((element) => ({ ...element, contentScaleY: Math.max(1, roundTo(toNumber(event.target.value, 1), 2)) }))} /></Field>
                <Field label="Movement"><select value={selectedElement.movementType || 'Clamped'} onChange={(event) => patchSelected((element) => ({ ...element, movementType: event.target.value }))}>{SCROLL_MOVEMENT_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field>
                <Field label="Inertia"><select value={selectedElement.inertia === false ? 'no' : 'yes'} onChange={(event) => patchSelected((element) => ({ ...element, inertia: event.target.value === 'yes' }))}><option value="yes">Yes</option><option value="no">No</option></select></Field>
                <Field label="Elasticity"><input type="number" min="0" step="0.01" value={selectedElement.elasticity ?? 0.1} onChange={(event) => patchSelected((element) => ({ ...element, elasticity: Math.max(0, roundTo(toNumber(event.target.value, 0.1), 3)) }))} /></Field>
                <Field label="Scroll Sensitivity"><input type="number" min="0" step="0.1" value={selectedElement.scrollSensitivity ?? 24} onChange={(event) => patchSelected((element) => ({ ...element, scrollSensitivity: Math.max(0, roundTo(toNumber(event.target.value, 24), 2)) }))} /></Field>
                <Field label="Deceleration" full><input type="number" min="0" step="0.001" value={selectedElement.decelerationRate ?? 0.135} onChange={(event) => patchSelected((element) => ({ ...element, decelerationRate: Math.max(0, roundTo(toNumber(event.target.value, 0.135), 3)) }))} /></Field>
              </>}
              {selectedElement.type === 'Countdown' && <>
                <Field label="Start Time"><input type="number" step="0.1" value={selectedElement.startTime ?? 0} onChange={(event) => patchSelected((element) => ({ ...element, startTime: roundTo(toNumber(event.target.value, 0), 2) }))} /></Field>
                <Field label="End Time"><input type="number" step="0.1" value={selectedElement.endTime ?? 30} onChange={(event) => patchSelected((element) => ({ ...element, endTime: roundTo(toNumber(event.target.value, 30), 2) }))} /></Field>
                <Field label="Interval"><input type="number" min="0.1" step="0.1" value={selectedElement.interval ?? 1} onChange={(event) => patchSelected((element) => ({ ...element, interval: Math.max(0.1, roundTo(toNumber(event.target.value, 1), 2)) }))} /></Field>
                <Field label="Timer Format"><select value={selectedElement.timerFormat || 'MinutesSeconds'} onChange={(event) => patchSelected((element) => ({ ...element, timerFormat: event.target.value }))}>{COUNTDOWN_FORMAT_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></Field>
                <Field label="Finish Command" full><input value={selectedElement.command || ''} onChange={(event) => patchSelected((element) => ({ ...element, command: event.target.value }))} placeholder="example.done" /></Field>
                <Field label="Destroy When Done"><select value={selectedElement.destroyIfDone ? 'yes' : 'no'} onChange={(event) => patchSelected((element) => ({ ...element, destroyIfDone: event.target.value === 'yes' }))}><option value="no">No</option><option value="yes">Yes</option></select></Field>
              </>}
              {selectedElement.type === 'Slot' && <Field label="Filter" full><input value={selectedElement.filter || ''} onChange={(event) => patchSelected((element) => ({ ...element, filter: event.target.value }))} placeholder="wood, stone, any" /></Field>}
